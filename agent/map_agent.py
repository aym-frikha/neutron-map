...
# class MAPPluginApi(agent_rpc.PluginApi,
...
class OVSNeutronOFPMAPAgent(n_rpc.RpcCallback,
    ...
    def __init__(self, integ_br, tunnel_ip, ovsdb_ip, ovsdb_port,
        ...
        super(OVSNeutronOFPMAPAgent, self).__init__()
        self.polling_interval = polling_interval
        self.agent_state = {
            'binary': 'neutron-openvswitch-agent',
            'host': "127.0.0.1",
            'topic': None,
            'configurations': {'bridge_mappings': None,
                               'tunnel_types': None,
                               'tunneling_ip': None,
                               'l2_population': None,
    ...
    def _setup_integration_br(self, root_helper, integ_br,
        ...
        #connect br-int to eth1 port
        self.int_br.add_port("eth1")
        self.int_br.add_port("tap-test")


        # rest_api_addr = self.plugin_rpc.get_ofp_rest_api_addr(self.context)
        # if not rest_api_addr:
        #     raise n_exc.Invalid(_("MAP rest API port isn't specified"))
...
def main():
    ...
    try:
        agent = OVSNeutronOFPMAPAgent(integ_br, tunnel_ip, ovsdb_ip,
    ...
    LOG.info(_("MAP initialization on the node is done. "
